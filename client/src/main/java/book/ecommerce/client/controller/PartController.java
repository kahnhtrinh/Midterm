package book.ecommerce.client.controller;
...
import book.ecommerce.library.service.CategoryService;
import book.ecommerce.library.service.MakeService;
import book.ecommerce.library.service.ModelService;
import book.ecommerce.library.service.ProductService;
import book.ecommerce.library.util.Utility;
...
public class PartController {
    ...
    public String partSearch(Model model
        ...
        List<Make> listMake = makeService.findAll().stream().sorted(Comparator.comparing(Make::getName)).collect(Collectors.toList());
        model.addAttribute("listMake", listMake);

        List<book.ecommerce.library.model.Model> listModel = modelService.getModels(Long.parseLong(makeId));
    ...
    public String partDetails(@RequestParam("id") Long id, Model model) {
        ...
        List<book.ecommerce.library.model.Model> listModel = modelService.getModels(listMake.get(0).getId());
        model.addAttribute("listModel", listModel);
